...
- **status**: **v0.15.0（Epic #88）で H1〜H6 すべて対応済み**。対応関係は
  H1→#89、H2→#94、H3→#96、H4→#90・#92、H5→#91・#97、H6→#93・#95。
  以下の症状・原因・実測の記述は対応前の観測時点の記録であり、歴史的記録として残す
  （書き換えていない）
...
**→ 対応済み（#89）**: v0.15.0 で generator への指示にベース合わせ手順を組み込んだ。
...
**→ 対応済み（#94）**: v0.15.0 で準備コマンドをレーンの作業ディレクトリ（isolation worktree）で
初回1回だけ実行するよう指示を改めた。
...
**→ 対応済み（#96）**: v0.15.0 で `cd` により作業ディレクトリを変えないことを明記した。
...
**→ 対応済み（#90, #92）**: v0.15.0 で epic skill の Task issue テンプレートに `- 前提:` を追加し、
`plan-waves.sh` の前提未宣言警告に件数と実効並列度を出すようにした。
...
**→ 対応済み（#91, #97）**: v0.15.0 で SKIP 件数を機械的に数える `scripts/count-skips.sh` を新設し、
レーン内ゲート・統合ゲートの SKIP 計測をこれに置き換えた。
...
**→ 対応済み（#93, #95）**: v0.15.0 で取り込み済みレーン worktree を片付ける
`scripts/cleanup-lane-worktrees.sh` を新設し、run の後片付けで当該 Epic 分のレーン worktree を
削除するようにした。
